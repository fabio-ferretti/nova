...
from listensleeponlan import listen_sleep_on_lan
from checklastvm import check_last_vm
from checkformigration import check_for_migration
...
class ComputeManager(manager.SchedulerDependentManager):
    ...
    def init_host(self):
        #TODO temporaneamente salvo le informzazioni sull'istanza in un file
        out_actions = ''.join([FLAGS.output_path, "output_actions"])
        out_file = open(out_actions,"a")
        out_file.write("\ninit_host \n")
        out_file.close()
        ...
        out_actions = ''.join([FLAGS.output_path, "output_actions"])
        out_file = open(out_actions,"a")
        out_file.write("\npre node_information \n")
        out_file.close()
        #Necessario per l'inizializzazione del sistema 
        node_informations(self.host, nova.context.get_admin_context())
        
        
        #TODO temporaneamente salvo le informzazioni sull'istanza in un file
        out_actions = ''.join([FLAGS.output_path, "output_actions"])
        out_file = open(out_actions,"a")
        out_file.write("\npre listen_sleep_on_lan \n")
        out_file.close()
        
        #Necessario per ascoltare le richieste di sospensione del sistema
        listen_sleep_on_lan()
        
        #TODO temporaneamente salvo le informzazioni sull'istanza in un file
        out_actions = ''.join([FLAGS.output_path, "output_actions"])
        out_file = open(out_actions,"a")
        out_file.write("\npost listen_sleep_on_lan \n")
        out_file.close()
    ...
    def run_instance(self, context, instance_uuid, **kwargs):
        ...
        def do_run_instance():
            #TODO temporaneamente salvo le informzazioni sull'istanza in un file
            #Inserire qui se sono necessarie azioni all'avvio di una VM
            out_actions = ''.join([FLAGS.output_path, "output_actions"])
            out_file = open(out_actions,"a")
            out_file.write("\nIstanza %s run \n" % instance_uuid)
            out_file.close()
    ...
    def terminate_instance(self, context, instance_uuid):
        ...
        def do_terminate_instance():
            ...
            try:
                ...
                result = check_last_vm(host,context)
                if result == True:
                    #controllo se e' possibile eseguire la live migration delle istanze verso gli altri nodi
                    check_for_migration(self,host,context)
